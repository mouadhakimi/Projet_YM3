...
<html lang="fr">
    <head>
        <meta charset="utf-8" />
        <title>Suppression Concours</title>
    ...
    <body>
        ...
        <div class="container-fluid">
            <div class="warning">
                ...
                <form method="post" action="suppression_4.php">
                <div class="input-group mb-3">
                    <span class="input-group-text" id="basic-addon1">N° concours :</span>
                    <select name="concours" class="form-control">
                    <?php
                            $sql = $conn->query("SELECT DISTINCT id_concours FROM concours");
                            while($row = $sql->fetch_array()){
                                echo '<option value="'.htmlspecialchars($row['id_concours']).'">'. htmlspecialchars($row['id_concours']).'</option>';
                            } 
                    ?>
                    </select>
                </div>
